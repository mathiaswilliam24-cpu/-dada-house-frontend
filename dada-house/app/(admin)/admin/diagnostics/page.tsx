...
export default function AdminDiagnosticsPage() {
  ...
  const load = useCallback(async (status: string) => {
    setLoading(true);
    const res = await fetch(`/api/admin/diagnostics?status=${status}`);
    if (res.status === 403) {
      setForbidden(true);
      setLoading(false);
      return;
  ...
  if (forbidden) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
        ...
        <p className="text-sm text-gray-400 mt-1">Diagnostic reviews are limited to Admin, Super Admin, and Manager roles.</p>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <ClipboardCheck className="w-6 h-6 text-[#1B3FA8]" /> Diagnostic Reviews
          </h1>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {rows.map((r) => (
                  <tr key={r.id} onClick={() => router.push(`/admin/diagnostics/${r.id}`)} className="hover:bg-gray-50 transition-colors cursor-pointer">
                    ...
                    <td className="px-4 py-3 text-xs text-gray-600">{r.repairUrgency?.replace(/_/g, " ") ?? "—"}</td>
